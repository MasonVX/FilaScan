...
# Source the ESP/xtensa toolchain so the xtensa-esp32s3-elf-gcc linker is on PATH.
# This file is generated by espup (run `espup install` to create/update it).
if [ -f "$HOME/export-esp190.sh" ]; then
  # shellcheck disable=SC1091
  . "$HOME/export-esp190.sh" >/dev/null 2>&1 || true
elif [ -f "$HOME/export-esp.sh" ]; then
  # shellcheck disable=SC1091
  . "$HOME/export-esp.sh" >/dev/null 2>&1 || true
fi
